...
export function CreativeCard({ creative }: CreativeCardProps) {
  return (
    <div
      className={`bg-white dark:bg-card/30 backdrop-blur-sm border border-slate-200 dark:border-border shadow-sm dark:shadow-none rounded-2xl p-5 flex flex-col group hover:border-[#FFBB00]/50 hover:shadow-md dark:hover:shadow-none transition-all duration-300 ${!creative.isActive ? "opacity-70" : ""
        }`}
    >
      ...
      <h3 className="text-lg font-bold mb-1 group-hover:text-[#FFBB00] transition-colors text-slate-900 dark:text-white">
        {creative.name}
      </h3>
      ...
      <div className="space-y-3 border-t border-slate-100 dark:border-border pt-4 mb-4">
        <div className="flex justify-between text-sm">
          <span className="text-slate-600 dark:text-muted-foreground">Lucro Total</span>
          <span className="font-bold text-slate-900 dark:text-white">{creative.totalProfitFormatted}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-slate-600 dark:text-muted-foreground">Lucro Freelancer</span>
          <span className="text-[#FFBB00] font-bold">
            {creative.freelancerCutFormatted}
          </span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-slate-600 dark:text-muted-foreground">Pago?</span>
          <Badge
            variant="outline"
            className={`text-xs font-bold ${Number(creative.totalProfit) === 0
                ...
                : "bg-amber-50 text-amber-700 border-amber-200 dark:bg-[#FFBB00]/10 dark:text-[#FFBB00] dark:border-[#FFBB00]/20"
              }`}
          >
      ...
      <div className="mt-auto pt-4 border-t border-slate-100 dark:border-border flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-xs text-slate-900 dark:text-white">{creative.operationName}</span>
        </div>
      </div>
    </div>
  );
