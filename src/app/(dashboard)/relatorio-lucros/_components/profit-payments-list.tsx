  ...
  if (groups.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
        <p className="text-sm">Nenhum pagamento encontrado para o período selecionado.</p>
      </div>
    );
  }
        ...
        >
          <AccordionTrigger className="hover:no-underline">
            <div className="flex flex-1 items-center justify-between pr-4">
              <span className="font-semibold text-base">{group.operationName}</span>
              <div className="flex items-center gap-6 text-sm">
                <div className="text-right hidden sm:block">
                  <p className="text-xs text-muted-foreground">Lucro total</p>
                  <p className="font-medium">{formatCurrency(group.totalLucro)}</p>
                </div>
          ...
          <AccordionContent>
            <div className="border-t pt-3 space-y-2">
              <div className="grid grid-cols-3 gap-4 px-2 pb-1 text-xs font-medium text-muted-foreground uppercase tracking-wide">
                <span>Criativo</span>
                <span className="text-right hidden sm:block">Lucro do criativo</span>
